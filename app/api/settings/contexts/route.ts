...
export async function GET() {
  ...
  const count = await ctx.db.agentContext.count();
  if (count === 0) {
    await ctx.db.agentContext.createMany({ data: DEFAULT_CONTEXTS });
  }

  const contexts = await ctx.db.agentContext.findMany({ orderBy: { order: "asc" } });
...
export async function POST(req: NextRequest) {
  ...
  const created = await ctx.db.agentContext.create({
...
export async function PUT(req: NextRequest) {
  ...
  const updated = await ctx.db.agentContext.update({
    where: { id },
...
export async function DELETE(req: NextRequest) {
  ...
  await ctx.db.agentContext.delete({ where: { id } });
